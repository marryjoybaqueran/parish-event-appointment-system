    ...
    <template #content>
      <v-container fluid class="pa-6">
        ...
        <v-row class="mb-6 mt-2">
          <v-col cols="12" md="3" v-for="(stat, idx) in [
            { icon: 'mdi-book-multiple', color: 'primary', value: stats.totalBookings, label: 'Total Bookings' },
            { icon: 'mdi-clock-alert', color: 'orange', value: stats.pendingApprovals, label: 'Pending Approvals' },
            { icon: 'mdi-calendar-check', color: 'green', value: stats.upcomingEvents, label: 'Upcoming Events' },
            { icon: 'mdi-account-group', color: 'purple', value: stats.totalMembers, label: 'Parish Members' }
          ]" :key="idx">
            <v-card class="stat-card card-shadow" elevation="3">
              <v-card-text class="text-center">
                <v-icon size="40" :color="stat.color" class="mb-3">{{ stat.icon }}</v-icon>
                <h2 class="text-h4 font-weight-bold">{{ stat.value }}</h2>
                <p class="text-grey-darken-1">{{ stat.label }}</p>
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>
