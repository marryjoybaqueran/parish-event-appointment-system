  ...
  async function getAnnouncementsFromApi() {
    ...
    const transformedData = announcementsFromApi.value.map((announcement) => {
      return {
        ...
        image_url: announcement.data?.['Image URL'] || '',
        date_posted: announcement.data?.['Date Posted'] || new Date().toISOString(),
        summary: announcement.data?.['Summary'] || '',
        details: announcement.data?.['Details'] || '',
        type: announcement.data?.['Event Type'] || '',
        priest: announcement.data?.['Priest'] || '',
        additional_info: announcement.data?.['Additional Info'] || {},
