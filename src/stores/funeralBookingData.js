...
export const useFuneralStore = defineStore('funeralData', {
  state: () => ({
    bookings: [],
    loading: false,
    error: null,
  ...
  actions: {
    ...
    // Create a new funeral booking (insert)
    // payload should be an object with the booking fields
    async createFuneral(payload = {}) {
      // reset formAction
      ...
      const { error } = await supabase.from('funeral_bookings').insert([insertPayload])
      ...
      this.formAction.formSuccessMessage = 'Funeral booking submitted successfully!'
      this.formAction.formProcess = false
      return true
    },

    // Fetch funeral bookings filtered by current user's ID
    async fetchUserFuneralBookings() {
      this.loading = true
      this.error = null
      ...
      try {
        ...
        this.bookings = data || []
        console.log(`Naka-fetch na ang ${this.bookings.length} funeral bookings para sa user`)
    ...
    // Fetch the most recent funeral booking for current user
    async fetchRecentFuneralBooking() {
      const user = await this.getUser()
      if (!user) {
        this.error = 'User not authenticated'
        return null
      ...
      try {
        ...
        if (recentBooking) {
          console.log('Recent funeral booking fetched:', recentBooking)
