...
export function useCameraThanksGivingUpload() {
  ...
  // Upload images to Supabase storage and update thanksgiving booking
  async function updateThanksGivingBookingWithImages(documentImages: Record<string, (string | { uri: string; id?: number; name?: string })[]>) {
    const user = await thanksGivingStore.getUser()
    if (!user) {
      return { error: 'User not authenticated' }
    }

    try {
      // Get most recent thanksgiving booking para sa user
      const recentBooking = await thanksGivingStore.fetchRecentThanksGivingBooking()
      if (!recentBooking) {
        return { error: 'No thanksgiving booking found para sa user' }
      }
      ...
      for (const [documentType, images] of Object.entries(documentImages)) {
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          const filename = `thanksgiving_${recentBooking.id}_${documentType}_${i + 1}_${timestamp}.png`
          const filePath = `thanksgiving_documents/${user.id}/${filename}`
      ...
      // Update thanksgiving booking with uploaded image URLs
      const imageUrls = {
        attached_images_1: uploadedUrls['certificate_of_confirmation']?.join(',') || null
      }
      ...
      if (error) {
        console.error('Error updating thanksgiving booking:', error)
      ...
      console.log('Successfully uploaded and updated thanksgiving booking with document images')
      return {
        data: data[0],
        uploadedUrls,
  ...
  return {
    updateThanksGivingBookingWithImages
  }
}
