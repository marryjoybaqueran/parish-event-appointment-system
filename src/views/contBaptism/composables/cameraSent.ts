...
export function useCameraBaptismUpload() {
  ...
  // Upload images to Supabase storage and update baptism booking
  async function updateBaptismBookingWithImages(documentImages: Record<string, (string | { uri: string; id?: number; name?: string })[]>) {
    const user = await baptismStore.getUser()
    if (!user) {
      return { error: 'User not authenticated' }
    }

    try {
      // Get most recent baptism booking para sa user
      const recentBooking = await baptismStore.fetchRecentBaptismBooking()
      if (!recentBooking) {
        return { error: 'No baptism booking found para sa user' }
      }
      ...
      for (const [documentType, images] of Object.entries(documentImages)) {
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          const filename = `baptism_${recentBooking.id}_${documentType}_${i + 1}_${timestamp}.png`
          const filePath = `baptism_documents/${user.id}/${filename}`
      ...
      // Update baptism booking with uploaded image URLs
      const imageUrls = {
        attached_images_1: uploadedUrls['permission_of_baptism']?.join(',') || null,
        attached_images_2: uploadedUrls['application_for_baptism']?.join(',') || null,
        attached_images_3: uploadedUrls['application_for_confirmation']?.join(',') || null
      }
      ...
      console.log('Successfully uploaded and updated baptism booking with document images')
      return {
        data: data[0],
        uploadedUrls,
  ...
  return {
    updateBaptismBookingWithImages
  }
}
