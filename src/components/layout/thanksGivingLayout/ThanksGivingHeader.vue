...
import { useThanksGivingHeader } from './thanksGiving'
...
<template>
  <v-container :class="mobile ? 'pa-2' : 'pa-4'" fluid>
    ...
    <v-alert v-if="thanksGivingStore.error" type="error" class="mb-4" closable>
      Error: {{ thanksGivingStore.error }}
    </v-alert>
        ...
        <template v-if="mobile">
          ...
          <div class="d-flex align-center justify-space-between">
              <div class="d-flex align-center">
                ...
                <span class="text-body-2">{{ formatDate(booking.thanksgiving_date) }}</span>
              </div>
          </div>
        </template>
        ...
        <template v-else>
          <v-row class="align-center">
            ...
            <v-col cols="3">
              <div class="d-flex align-center">
                <v-icon color="primary" size="small" class="mr-2">mdi-calendar</v-icon>
                <span class="text-body-1">{{ formatDate(booking.thanksgiving_date) }}</span>
              </div>
            </v-col>
